...
TexturedMeshVisual::TexturedMeshVisual(Ogre::SceneManager* scene_manager,
  ...
  vtx_shader_ =
      Ogre::HighLevelGpuProgramManager::getSingletonPtr()->createProgram(
          "vertex_shader",
          Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "glsl",
  ...
  vparams->setNamedAutoConstant(
      "MVP",
      Ogre::GpuProgramParameters::AutoConstantType::ACT_WORLDVIEWPROJ_MATRIX);
  vparams->setNamedAutoConstant(
      "MV", Ogre::GpuProgramParameters::AutoConstantType::ACT_WORLDVIEW_MATRIX);
  ...
  jet_shader_ =
      Ogre::HighLevelGpuProgramManager::getSingletonPtr()->createProgram(
          "jet_shader", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME,
...
void TexturedMeshVisual::setFromMessage(
...
void TexturedMeshVisual::createMesh(
  ...
  while (field_idx < fields.size()) {
    ...
    if (name == "x") {
      ...
      vtx_dec->addElement(0, fields[field_idx].offset, Ogre::VET_FLOAT3,
                          Ogre::VES_POSITION);
      field_idx += 3;
      ...
      vtx_dec->addElement(0, fields[field_idx].offset, Ogre::VET_FLOAT2,
...
void TexturedMeshVisual::updateVertexBuffer(
    const Ogre::MeshPtr& mesh, const sensor_msgs::PointCloud2& cloud) {
  ...
  if ((vtx_buffer.isNull()) ||
      (vtx_buffer->getSizeInBytes() < cloud.data.size())) {
    // Create a new vertex buffer.
    Ogre::HardwareBufferManager& hw_manager =
        Ogre::HardwareBufferManager::getSingleton();
    vtx_buffer = hw_manager.createVertexBuffer(
...
void TexturedMeshVisual::updateIndexBuffer(
    const Ogre::MeshPtr& mesh, const std::vector<pcl_msgs::Vertices>& indices) {
  ...
  if (idx_buffer.isNull() ||
    ...
    Ogre::HardwareBufferManager& hw_manager =
        Ogre::HardwareBufferManager::getSingleton();
    idx_buffer = hw_manager.createIndexBuffer(
        Ogre::HardwareIndexBuffer::IT_32BIT, indices.size() * 3,
        Ogre::HardwareBuffer::HBU_STATIC_WRITE_ONLY);
    sub_mesh->indexData->indexBuffer = idx_buffer;
  }

  // Lock buffer and grab pointer to data.
  uint32_t* idx_data = static_cast<uint32_t*>(
